...
void part2cell_cic(struct PART *curp, struct OCT *curoct, int icell, char full)
{
  ...
  if(full){
    ...
    for(i1=0;i1<3;i1++){
      ...
      contrib=vcont[idx1];
      if(vnei[neip[i1]]==6){
	curoct->cell[vcell[neip[i1]]].density+=contrib/pow(dxcur,3)*curp->mass;
	...
	if(curoct->nei[vnei[neip[i1]]]->child!=NULL){
	  curoct->nei[vnei[neip[i1]]]->child->cell[vcell[neip[i1]]].density+=contrib/pow(dxcur,3)*curp->mass;
	...
	for(i2=0;i2<3;i2++){
	  ...
	  if(vnei2[neip[i2]]==6){
	    newoct->cell[vcell2[neip[i2]]].density+=contrib/pow(dxcur,3)*curp->mass;
	    ...
	    if(newoct->nei[vnei2[neip[i2]]]->child!=NULL){
	      newoct->nei[vnei2[neip[i2]]]->child->cell[vcell2[neip[i2]]].density+=contrib/pow(dxcur,3)*curp->mass;
	...
	if(newcell2!=NULL){
	  for(i3=0;i3<3;i3++){
	    ...
	    if(vnei3[neip[i3]]==6){
	      newoct2->cell[vcell3[neip[i3]]].density+=contrib/pow(dxcur,3)*curp->mass;
	      visit[7]=1;
	      tot+=contrib;
	      ntot++;
		...
		newoct2->nei[vnei3[neip[i3]]]->child->cell[vcell3[neip[i3]]].density+=contrib/pow(dxcur,3)*curp->mass;
		visit[7]=1;
		tot+=contrib;
		ntot++;
	      }
