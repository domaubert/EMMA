...
func readsnap(name,nbnd=,keep=,light=,ngrp=)
{
  ...
  if(!light){
  egy=array(float,ncells*ncells*ncells*ngrp);_read,ff,adress,egy;adress+=sizeof(egy);
  flx=array(float,3*ncells*ncells*ncells*ngrp);_read,ff,adress,flx;adress+=sizeof(flx);
  ...
  if(!keep){
    if(!light){
      egy=reform(egy,[4,ncells,ncells,ncells,ngrp]);
      egy=egy(nbnd+1:ncells-nbnd,nbnd+1:ncells-nbnd,nbnd+1:ncells-nbnd,);
      flx=reform(flx,[5,ncells,ncells,ncells,3,ngrp]);
...
func buildalist(rootname,outname,ifirst=,ilast=,nbnd=,light=){
  ...
  for(i=1;i<=ilast;i++){
    if(i>=ifirst){
      s=readhdr(ll(i-ifirst+1),nbnd=nbnd,light=light);
      a=s.aexp;
    }
    else{
      a=-1000.;
...
func read1024_light(fname,nbnd=,skip=){
  if(is_void(nbnd)) nbnd=16;
  ...
  field=array(float,ncells,ncells,ncells);
  ...
  //fa=reform(field,[3,ncells,ncells,ncells]);
  fa=field(nbnd+1:ncells-nbnd,nbnd+1:ncells-nbnd,nbnd+1:ncells-nbnd);
  s.src=&fa;
  fa=[];
...
func read1024(fname,nbnd=,skip=){
  ...
  // skipping egy, flx, xion

  adress+=5*sizeof(float)*ncells^3;
