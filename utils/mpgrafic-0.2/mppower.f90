...
function p(ak,a)
  ...
  elseif (icase.eq.5) then
     ...
     omhh = omegam*h*h
     f_baryon=omegab/omegam
     t=TFmdm_onek_mpc_bis(ak,omhh,f_baryon)
...
function pcdm(ak,a)
  ...
  if (icase.eq.3) then
     pcdm=p(ak,a)
     return
  else if (icase.eq.2.or.icase.eq.4.or.icase.eq.5) then
     !**************************************
     !  Transfer function case.  The transfer function T(k) is defined so
     !  that for initial curvature perturbations, phi(k,a=1)=T(k)*psi(k,a=0)
     !  where psi(k,a=0) has power spectrum pnorm*ak**(an-4) and phi(k,a=1)
...
subroutine pini
  ...
  if (icase.eq.1) then
     ...
     !  Tcmb in micro-K.
     tcmb=tcmb*1.e6
  else if (icase.eq.2.or.icase.eq.3.or.icase.eq.4.or.icase.eq.5) then
     tcmb=2.726e6
     if (h0.eq.0) then
  ...
  !**************************************
  if (icase.eq.2.or.icase.eq.4.or.icase.eq.5) then
...
40	rombin=g0
	...
	return
     ...
     !ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
     real function TFmdm_onek_mpc_bis(k,omhh,f_baryon)

       !  Calculate transfer function wiggle-less
       !
       !  Input: 
       !	 k -- wavenumber in Mpc^{-1}  
       !        omhh -- The density of CDM and baryons, in units of critical dens,
       ...
       real y,omhh,obhh,Tcmb,s_tilde,TF_pressureless,TF_pressureless_b
       ...
       !q = k/13.41/k_equality
       alpha_g=1.-0.328*Log(431.*omhh)*f_baryon+0.38*Log(22.3*omhh)*f_baryon**2
       ks = k*sound_horizon
       q = k*theta_cmb**2./omhh;
       q = q/(alpha_g+(1.-alpha_g)/(1.+(0.43*ks)**4.))
       ...
       tf_baryon = TF_pressureless_b(q)
       tf_full = f_baryon*tf_baryon + (1-f_baryon)*tf_cdm

       TFmdm_onek_mpc_bis=tf_full;


       return 
     end function TFmdm_onek_mpc_bis
     ...
     real function TF_pressureless_b(q)
       ...
       L0=Log(exp(1.)*2.+1.8*q)
       
       TF_pressureless_b = L0/(L0+C0*q**2)

       return

     end function TF_pressureless_b
