...
#define  rfftwnd_f77_mpi_local_sizes rfftwnd_f77_mpi_local_sizes_
  ...
  integer ipad, nxs, nys, nzs, iseeds
...
!!$       &                         dxr,1.0d-7)
  ...
  if (myid==0) then
     ...
     print*,'  when sigma(delta) at ultimate refinement scale=' &
          &          ,real(sigma)
     print*,'D+(astart)=',dplus(astart,omegam,omegav)&
          &,'D+(1.0)=',dplus(1.0,omegam,omegav) &
          &,'ratio=',(dplus(astart,omegam,omegav)&
          &/dplus(1.0,omegam,omegav))**2
     open(10,file='dplus.dat',form='formatted',status='unknown')
     rewind 10
     write(10,*) dplus(astart,omegam,omegav)&
          &,dplus(1.0,omegam,omegav)&
          &,(dplus(astart,omegam,omegav)/dplus&
          &(1.0,omegam,omegav))**2
     close(10)

     open(10,file='dplustab.dat',form='formatted',status='unknown')
     rewind 10
     do itab=0,256
        atab=0.01+itab*(1.-0.01)/256
        write(10,*) atab,dplus(atab,omegam,omegav)
     end do
     close(10)
  endif
