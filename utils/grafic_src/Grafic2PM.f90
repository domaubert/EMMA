...
program Grafic2Gadget
  ...
  integer*4::ifile,nmult=1,nmult2,nmultx=1,nmulty=1,nmultz=1
  integer*4::ifilex,ifiley,ifilez
  integer*4::start1,start2,start3
  integer*4::stop1,stop2,stop3

  character*80::fname,nmult2s,nmult2sx,nmult2sy,nmult2sz
  character*2::dummy2
  character*1::dummy1
  real::ii
  integer::count
  ...
  CALL getarg(3,nmult2sx)
  CALL getarg(4,nmult2sy)
  CALL getarg(5,nmult2sz)

  if(trim(nmult2sx).ne.'') then
     read(nmult2sx,*) nmultx
     read(nmult2sy,*) nmulty
     read(nmult2sz,*) nmultz
     nmult=nmultx*nmulty*nmultz
  end if


  write(*,*) nmultx,nmulty,nmultz
  ...
  write(*,*) '# of files Grafic2Gadget will output: ',nmultx*nmulty*nmultz 
  write(*,*) '**********************************************'
  ...
  allocate(velx(np1,np2,np3))
  allocate(vely(np1,np2,np3))
  allocate(velz(np1,np2,np3))

  allocate(posx(np1,np2,np3))
  allocate(posy(np1,np2,np3))
  allocate(posz(np1,np2,np3))
...
!==========================================================================================
  write(*,*) nmultx,nmulty,nmultz
  write (*,*) posx(1,1,1),posy(1,1,1),posz(1,1,1)
  do ifilex=0,nmultx-1
     do ifiley=0,nmulty-1
        do ifilez=0,nmultz-1
     
           write(*,*) ifilex,ifiley,ifilez
           ifile=ifilex+ifiley*nmultx+ifilez*nmultx*nmulty
           
           ! GENERATING NAMES =====================================
           ! + OPEN FILES
           
           if(ifile.ge.10) then
              write(dummy2,'(i2)') ifile
              fname=trim(output)//'IC.PM.'//dummy2
           else
              write(dummy1,'(i1)') ifile
           ...
           start1=ifilex*np1/nmultx+1
           stop1=(ifilex+1)*np1/nmultx-1+1
           
           start2=ifileY*np2/nmulty+1
           stop2=(ifiley+1)*np2/nmulty-1+1
           
           start3=ifilez*np3/nmultz+1
           stop3=(ifilez+1)*np3/nmultz-1+1
           ...
           write(100+ifile) (((posx(i1,i2,i3)-start1+1,i1=start1,stop1),i2=start2,stop2),i3=start3,stop3)
           write(100+ifile) (((posy(i1,i2,i3)-start2+1,i1=start1,stop1),i2=start2,stop2),i3=start3,stop3)
           write(100+ifile) (((posz(i1,i2,i3)-start3+1,i1=start1,stop1),i2=start2,stop2),i3=start3,stop3)
           
           ! WRITING VELOCITIES =====================================
           
           write(100+ifile) (((velx(i1,i2,i3),i1=start1,stop1),i2=start2,stop2),i3=start3,stop3)
           write(100+ifile) (((vely(i1,i2,i3),i1=start1,stop1),i2=start2,stop2),i3=start3,stop3)
           write(100+ifile) (((velz(i1,i2,i3),i1=start1,stop1),i2=start2,stop2),i3=start3,stop3)
