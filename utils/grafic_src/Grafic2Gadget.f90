...
program Grafic2Gadget
  ...
  IF(narg .LT. 2)THEN
     ...
     write(*,*)'and directory output will receive the file IC.gad'
  ...
  write(*,*) 'mass per particle (1e10 Ms)= ',massarr_out(1)


  velx=velx*1000.*hubble_out
  vely=vely*1000.*hubble_out   ! GADGET LENGTH UNITS ARE IN KPC H-1
  velz=velz*1000.*hubble_out
  ...
  do ifile=1,nmult
     ...
     if(ifile-1.ge.10) then
        ...
        fname=trim(output)//'IC.gad.'//dummy2
     else
        write(dummy1,'(i1)') ifile-1
        fname=trim(output)//'IC.gad.'//dummy1
     end if
     ...
     do i3=1,np3/nmult
        ...
        do i2=1,np2
           ...
           do i1=1,np1
              ...
              velx(i1,i2,i3)=x0+velx(i1,i2,i3)/vfact
              vely(i1,i2,i3)=y0+vely(i1,i2,i3)/vfact
              velz(i1,i2,i3)=z0+velz(i1,i2,i3)/vfact
     ...
     velx=velx*1000.*hubble_out !
     vely=vely*1000.*hubble_out ! Length units are h-1*kpc
     velz=velz*1000.*hubble_out !
     ...
     write(100+ifile) (npart_out(i),i=0,5), (massarr_out(i),i=0,5), a_out, &
          redshift_out, flag_sfr_out, flag_feedback_out,  &
          (nall_out(i),i=0,5), flag_cooling_out, numfiles_out, &
          xLbox_out,omega0_out,omegaL_out,hubble_out, &
          (unused_out(i),i=1,64-6-12-4-8-2-4*2)
     ...
     do i3=1,np3/nmult
        ...
        do i2=1,np2
           ...
           do i1=1,np1
              ...
              velx(i1,i2,i3)=(-x0+velx(i1,i2,i3)/1000./hubble_out)/sqrt(astart)*vfact
              vely(i1,i2,i3)=(-y0+vely(i1,i2,i3)/1000./hubble_out)/sqrt(astart)*vfact
              velz(i1,i2,i3)=(-z0+velz(i1,i2,i3)/1000./hubble_out)/sqrt(astart)*vfact
     ...
     write(*,*) 'average vel=',sumv/(np1*np2*np3)
     ...
     write(100+ifile) (i+np1*np2*np3/nmult*(ifile-1),i=1,np1*np2*np3/nmult)
