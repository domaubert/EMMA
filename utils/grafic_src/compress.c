/* flgic.c has been modified: ReadValue replaced by data, len.
 * Also, move static cur_bits and cur_accum to front and initialize
 * in compress.  * EB, 6/21/95.  */
 ...
 *
 * CONTENTS
XXX *	compress( init_bits, outfile, ReadValue ) XXX
...
static	output(), cl_block(), cl_hash(), writeerr(), char_init(), char_out(),
	flush_char();
...
int len;
{
    ...
    cur_bits = 0;			/* Initialize output buffer */
    cur_accum = 0;			/* Added by EB, 6/21/95 */
