...
__device__ REAL dfindPressure(struct Wtype1D *WL, struct Wtype1D *WR, int *niter, REAL *u)
{
  ...
  *niter=0;
  for(i=0;i<NITERMAX;i++)
    {
      dp=dfroot(p,WL,WR,&u2)/dfrootprime(p,WL,WR);///fmax(0.8,fmin(1.2,(1.0-0.5*dfroot(p,WL,WR,u)*dfrootprimeprime(p,WL,WR)/dfrootprime(p,WL,WR))));
      if((isnan(dp))){
      	//abort();
      }
      ...
      porg=p;
      p=p-dp;
      //if(dfrootprime(p,WL,WR)==0) abort();//printf("p0=%e dp=%e p=%e fprime=%e\n",porg,dp,p,dfrootprime(p,WL,WR));
...
//============================================================================
//============================================================================
__global__ void dev_hydroM_sweepX(struct HGRID *stencil, int nread,REAL dx, REAL dt){
...
#ifdef DUAL_E
	ebar=(Us.E-0.5*(Us.dv*Us.dv+Us.dw*Us.dw+Us.du*Us.du)/Us.d);
	ecen+=ebar;
...
// ==========================================================================
__global__ void dev_hydroM_sweepY(struct HGRID *stencil, int nread,REAL dx, REAL dt){
...
__global__ void dev_hydroM_sweepZ(struct HGRID *stencil, int nread,REAL dx, REAL dt){
  ...
  for(icell=0;icell<8;icell++){ // we scan the cells
    ...
    for(iface=0;iface<2;iface++){
      ...
      WC[iface].d=WC[inei].d;
      WC[iface].u=WC[inei].u;
      WC[iface].v=WC[inei].v;
      WC[iface].w=WC[inei].w;
      WC[iface].p=WC[inei].p;
      WC[iface].a=WC[inei].a;
    ...
    for(iface=0;iface<2;iface++){
      ...
      WN[iface].d=WT[idxnei[inei]].d;
      WN[iface].u=WT[idxnei[inei]].u;
      WN[iface].v=WT[idxnei[inei]].v;
      WN[iface].w=WT[idxnei[inei]].w;
      WN[iface].p=WT[idxnei[inei]].p;
      WN[iface].a=WT[idxnei[inei]].a;
    ...
    //========================= copy the fluxes
    int j;
    for(j=0;j<NVAR;j++) stencil[i].New.cell[icell].flux[4*NVAR+j]=FL[j];
    for(j=0;j<NVAR;j++) stencil[i].New.cell[icell].flux[5*NVAR+j]=FR[j];
...
__global__ void dev_updatefield(struct HGRID *stencil, int nread, int stride, struct CPUINFO *cpu, REAL dxcur, REAL dtnew)
{
  int i,icell;
  struct Utype U;
  REAL one;
  ...
  for(icell=0;icell<8;icell++){ // we scan the cells
    ...
    stencil[i].New.cell[icell].deltaU.d =U.d;
    stencil[i].New.cell[icell].deltaU.du=U.du;
    stencil[i].New.cell[icell].deltaU.dv=U.dv;
    stencil[i].New.cell[icell].deltaU.dw=U.dw;
    stencil[i].New.cell[icell].deltaU.E =U.E;
#ifdef DUAL_E
    stencil[i].New.cell[icell].deltaU.eint =U.eint;
#endif
  }
}
...
int advancehydroGPU(struct OCT **firstoct, int level, struct CPUINFO *cpu, struct HGRID *stencil, int stride, REAL dxcur, REAL dtnew){
  ...
  if((nextoct!=NULL)&&(cpu->noct[level-1]!=0)){
	...
	// ------------ solving the hydro
	dev_hydroM_sweepX<<<gridoct,blockoct,0,stream[is]>>>(cpu->hyd_stencil+offset,nread/cpu->nstream,dxcur,dtnew);
	dev_hydroM_sweepY<<<gridoct,blockoct,0,stream[is]>>>(cpu->hyd_stencil+offset,nread/cpu->nstream,dxcur,dtnew); 
	dev_hydroM_sweepZ<<<gridoct,blockoct,0,stream[is]>>>(cpu->hyd_stencil+offset,nread/cpu->nstream,dxcur,dtnew); 
	
	// ------------ updating values within the stencil
	/* cudaDeviceSynchronize(); */
	/* t[4]=MPI_Wtime(); */

	 dev_updatefield<<<gridoct,blockoct,0,stream[is]>>>(cpu->hyd_stencil+offset,nread,stride,cpu,dxcur,dtnew);
